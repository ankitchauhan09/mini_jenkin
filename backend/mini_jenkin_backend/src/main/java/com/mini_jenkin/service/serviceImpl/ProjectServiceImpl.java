...
public class ProjectServiceImpl implements ProjectServiceInterface {
    ...
    @Override
    public Project updatePipeline(PipelineConfig pipelineConfig, Long pipelineId) {
        PipelineConfig oldConfig = pipelineRepository.findById(pipelineId)
                .orElseThrow(() -> new ResourceNotFoundException("Pipeline with ID: " + pipelineId + " not found"));

        // Update name if provided
        if (pipelineConfig.getName() != null) {
            oldConfig.setName(pipelineConfig.getName());
        }

        // Update stages if provided
        if (pipelineConfig.getStages() != null && !pipelineConfig.getStages().isEmpty()) {
            oldConfig.getStages().clear();
            pipelineConfig.getStages().forEach(stage -> {
                stage.setPipelineconfig(oldConfig);
                oldConfig.getStages().add(stage);
            });
        }

        return pipelineRepository.save(oldConfig).getProject();
    }
